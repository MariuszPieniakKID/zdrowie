...
import type { WykresWynikowProps, Parameter } from './types';
...
function WykresWynikow({
  ...
  const uniqueParams = Array.from(new Set(parameters.map(p => p.parameter_name)));
  const [isParametersCollapsed, setIsParametersCollapsed] = useState<boolean>(uniqueParams.length > 8);

  const generateAnalysis = (paramName: string, value: string, units?: string): string => {
  ...
  const groupedData = parameters.reduce<Record<string, { values: Record<string, string>; units?: string; analysis: Record<string, string> }>>((acc, param) => {
  ...
  const uniqueDates = Array.from(new Set(parameters.map(p =>
  ...
  const getCellProps = (analysisInput?: string): { bg: string; arrow: string | null; color?: string } => {
