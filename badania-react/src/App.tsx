...
function App() {
  ...
  const [awaitingSmsCode, setAwaitingSmsCode] = useState<boolean>(false);
  const [smsCodeId, setSmsCodeId] = useState<string | null>(null);
  ...
  const fetchFiles = useCallback(async () => {
    if (!user?.id) return;
    try {
      const res = await axios.get<FilesData>(`https://zdrowie-backend.vercel.app/api/documents/${user.id}?page=${files.page}`);
      setFiles(res.data);
    } catch (err) {
  ...
  const fetchParameters = useCallback(async () => {
    if (!user?.id) return;
    try {
      const res = await axios.get<Parameter[]>(`https://zdrowie-backend.vercel.app/api/parameters/${user.id}`);
  ...
  const generateChartData = useCallback(() => {
    ...
    parameters.forEach(param => {
      ...
      const xVal = new Date(param.measurement_date);
  ...
  useEffect(() => {
    const checkExistingAnalysis = async () => {
      try {
        const res = await axios.get<FilesData>(`https://zdrowie-backend.vercel.app/api/documents/${user!.id}`);
  ...
  const isValidPhone = (phone: string): boolean => /^(\+48)?\d{9}$/.test(sanitizePhone(phone));
  ...
  const handleSendSmsCode = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setError('');
    ...
    setLoading(true);
    try {
      const res = await axios.post<{ codeId: string }>('https://zdrowie-backend.vercel.app/api/send-sms-code', { 
  ...
  const handleVerifySmsCode = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setError('');
    ...
    setLoading(true);
    try {
      const res = await axios.post<{ user: User }>('https://zdrowie-backend.vercel.app/api/verify-sms-code', {
        phone: sanitizePhone(loginPhone),
        code: smsCode,
        codeId: smsCodeId
      });
  ...
  const handleAnalyze = async (doc: Document) => {
    ...
    setAnalysis('');
    try {
      const res = await axios.post<{ analysis: string }>('https://zdrowie-backend.vercel.app/api/analyze-file', {
        document_id: doc.id,
        user_id: user!.id
      });
  ...
  const handleSelectAll = () => {
    const allParams = Array.from(new Set(parameters.map(p => p.parameter_name)));
  ...
  const handleSummarize = async () => {
    ...
    setIsAnalyzing(true);
    try {
      const res = await axios.post<{ summary: string }>('https://zdrowie-backend.vercel.app/api/summarize', {
        userId: user!.id,
